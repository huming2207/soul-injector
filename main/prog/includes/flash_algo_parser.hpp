...
public:
    ...
    esp_err_t get_test_description(flash_algo::test_description *descr);
    esp_err_t get_dev_description(flash_algo::dev_description *descr);
    esp_err_t get_flash_algo(uint8_t *buf_out, size_t buf_len, size_t *actual_len);

private:
    esp_err_t get_section_data(void *data_out, const char *section_name, size_t min_size, size_t *actual_size) const;
