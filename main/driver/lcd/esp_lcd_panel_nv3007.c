...
esp_err_t esp_lcd_new_panel_nv3007(const esp_lcd_panel_io_handle_t io,
                                   ...
                                   esp_lcd_panel_handle_t *ret_panel)
{
    ...
    ESP_GOTO_ON_FALSE(panel_dev_config->data_endian == LCD_RGB_DATA_ENDIAN_BIG,
                      ESP_ERR_NOT_SUPPORTED, err, TAG, "unsupported RGB data endian");
    ESP_GOTO_ON_FALSE(panel_dev_config->bits_per_pixel == 16,
                      ESP_ERR_NOT_SUPPORTED, err, TAG, "unsupported pixel width");
...
static esp_err_t panel_nv3007_reset(esp_lcd_panel_t *panel)
{
    ...
    if (nv3007->reset_gpio_num >= 0) {
        ...
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(nv3007->io, LCD_CMD_SWRESET, NULL, 0), TAG,
                            "io tx param failed");
        vTaskDelay(pdMS_TO_TICKS(120));
    }
...
static esp_err_t panel_nv3007_init(esp_lcd_panel_t *panel)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(nv3007->io, LCD_CMD_MADCTL, &nv3007->madctl_val, 1), TAG,
                        "io tx param failed");
                                          ...
                                          const void *color_data)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(nv3007->io, LCD_CMD_CASET, (uint8_t[]) {
        (x_start >> 8) & 0xff,
        x_start & 0xff,
        ((x_end - 1) >> 8) & 0xff,
        (x_end - 1) & 0xff,
    }, 4), TAG, "io tx param failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(nv3007->io, LCD_CMD_RASET, (uint8_t[]) {
        (y_start >> 8) & 0xff,
        y_start & 0xff,
        ((y_end - 1) >> 8) & 0xff,
        (y_end - 1) & 0xff,
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_color(nv3007->io, LCD_CMD_RAMWR, color_data, len), TAG,
                        "io tx color failed");
    return ESP_OK;
}
...
static esp_err_t panel_nv3007_disp_on_off(esp_lcd_panel_t *panel, bool on_off)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(nv3007->io, command, NULL, 0), TAG,
                        "io tx param failed");
    return ESP_OK;
}
