...
esp_err_t ui_screen_progress_279::init(lv_obj_t *_base_obj)
{
    ...
    lv_obj_set_size(progress_bar, 428, 142);
    ...
    lv_obj_set_size(header_label, 428, 36);
    ...
    lv_obj_set_size(comment_label, 428, 89);
...
void ui_screen_progress_279::deinit()
{
    if (base_obj != nullptr) {
        lv_obj_del(base_obj);
    }
...
void ui_screen_progress_279::set_progress(uint32_t done, uint32_t full)
{
    if (progress_bar == nullptr) {
        return;
    }
...
void ui_screen_progress_279::set_header_text(const char *text)
{
...
void ui_screen_progress_279::set_comment_text(const char *text)
{
...
void ui_screen_progress_279::set_progress_bar_color(lv_color_t indicator_color, lv_color_t bg_color )
{
    if (progress_bar == nullptr) {
        return;
    }
