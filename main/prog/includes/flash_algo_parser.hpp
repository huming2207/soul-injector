...
public:
    ...
    esp_err_t get_flash_algo(uint8_t *buf_out, size_t buf_len, size_t *actual_len, uint32_t *code_start_addr = 0) const;
    esp_err_t get_func_pc(const char *func_name, uint32_t *pc_out);
    esp_err_t get_section_data(void *data_out, const char *section_name,  size_t min_size, size_t *actual_size,
                               uint32_t offset = 0, uint32_t *start_addr = nullptr) const;
