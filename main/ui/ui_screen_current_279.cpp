...
esp_err_t ui_screen_current_279::init(lv_obj_t *_base_obj)
{
    ...
    lv_obj_set_pos(current_label, 0, 37);
    lv_obj_set_size(current_label, 428, 57);
    ...
    lv_obj_set_pos(state_label, 0, 94);
    lv_obj_set_size(state_label, 428, 48);
...
void ui_screen_current_279::deinit()
{
    if (base_obj != nullptr) {
        lv_obj_del(base_obj);
    }
...
void ui_screen_current_279::set_current_main(const char *text)
{
    if (current_label == nullptr) {
        return;
    }
...
void ui_screen_current_279::set_state(const char *text, lv_color_t bg_color)
{
    if (state_label == nullptr) {
        return;
    }
